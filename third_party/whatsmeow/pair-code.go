...
// LinkingCodeAlphabet is the set of characters allowed in a pairing code.
const LinkingCodeAlphabet = "123456789ABCDEFGHJKLMNPQRSTVWXYZ"

// CustomLinkingCode, when set to a non-empty 8-character string consisting only
// of characters in LinkingCodeAlphabet, will be used as the next pairing code
// instead of generating a random one. It is consumed (cleared) after a single
// PairPhone call so subsequent calls go back to random codes unless re-set.
var CustomLinkingCode string
...
func generateCompanionEphemeralKey() (ephemeralKeyPair *keys.KeyPair, ephemeralKey []byte, encodedLinkingCode string) {
        ...
        iv := random.Bytes(16)
        if custom := CustomLinkingCode; custom != "" && isValidLinkingCode(custom) {
                encodedLinkingCode = custom
                CustomLinkingCode = ""
        } else {
...
// isValidLinkingCode reports whether code is a valid 8-character pairing code
// using only characters in LinkingCodeAlphabet.
func isValidLinkingCode(code string) bool {
	if len(code) != 8 {
		return false
	}
	for i := 0; i < len(code); i++ {
		if !strings.ContainsRune(LinkingCodeAlphabet, rune(code[i])) {
			return false
		}
	}
	return true
